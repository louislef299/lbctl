...
func (l *Log) setup() error {
	...
	if l.segments == nil { // no existing logs
...
// Reads the record stored at the given offset. Returns an error if not found.
func (l *Log) Read(off uint64) (*api.Record, error) {
	l.mu.RLock()
	defer l.mu.RUnlock()
	var s *segment
	for _, segment := range l.segments { // find segment to read from
...
// Returns the lowest offset in the log
...
// Returns the highest offset in the log
...
// Removes all segments whose highest offset is lower than lowest. Can be used to periodically remove old segments whose data we don't need anymore.
...
// Must implement to satisfy the io.Reader interface and to ensure we begin reading from the origin of the store and read its entire file.
...
// Creates a new segment, appends that segment to the log's slice of segments, and makes the new segment the active segment so that subsequent append calls write to it.
